...
LastEditTime: 2020-12-31 19:34:00
'''
...
def _integrate_over_pdf(hydrom, hydrom_type, frequency, elevation, temperature, list_D, list_beta, list_asp, levelb_name_folder):
    ...
    # print(weights_asp.sel(lambda_a=6.0, lambda_b=-0.6, mu_a=0.055, mu_b=0.8, method='nearest'))
    ...
    for var in ds_in.data_vars.keys():
        ...
        mean = (padded_da*weights_beta).sum(dim='beta', skipna=False)
        if ARS[hydrom_type] != 'SINGLE':
            mean = (mean*weights_asp).sum(dim='aspect_ratio', skipna=False)
        datadic[var] = mean
        
    ds_out = xr.Dataset(datadic, coords=coords)

    # print(ds_out.data_vars['p12_bw'].sel(std_a=40.0, std_b=-0.08, lambda_a=6.0, 
    ...
    with xr.open_dataset(levela_name_lut, engine="h5netcdf") as levela_lut:
        ...
        for e in list_elevation:
            for t in list_temperature:
                # _integrate_over_pdf(hydrom, hydrom_type, frequency, e, t, list_D, list_beta, list_asp, levelb_name_folder)
                # exit()
        ...
        # _integrate_over_pdf(hydrom, hydrom_type, frequency, 1., 253., list_D, list_beta, list_asp, levelb_name_folder)
